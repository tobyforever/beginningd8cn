...
基于Drupal的网站上的资讯内容(Content,直接翻译为“内容”，为了避免和下文的body的“内容”含义混淆，Content下面意译为“资讯内容”)。资讯内容一般是由一个标题开头，然后跟着正文内容。在第一章,我们创建一个带有标题和正文内容的基本资讯内容。Drupal提供了扩展和自定义资讯内容类型的能力。自定义资讯内容类型允许您创建额外的字段，可以用来记录其他相关信息。一个常见的例子是一个日历事件。
一个事件（Event）包括标题和正文(事件的描述),以及其他相关信息,如：日期和时间、地点,甚至地图或照片。我将在第5章介绍如何创建自定义内容类型。

##在Drupal中创建内容
在第一章中,我通过演示如何创建您的第一个内容项并发布到你的网站来演示了Drupal的内容创建功能。第一章中所使用的资讯类型是“基本页面”（basic page）。 Drupal 8还有第二个内容类型:文章（Article）。一篇文章相当于一个基本页面，唯独不相同的是文章有图像载入功能，并且文章的作者可以为文章输入一个称为标签（tag）的附加属性。标签是一些简单的词，有利于对站点里的内容进行分类，整理和搜索。这是Drupal的一个强大功能，我们会在第四章详细讨论它。
